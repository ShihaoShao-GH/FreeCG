...
from freecg.models.utils import (
    CosineCutoff,
    Distance, 
    EdgeEmbedding,
    NeighborEmbedding, 
...
class FreeCGBlock(nn.Module):
    ...
    def __init__(
        ...
        vertex_type="Edge",
        num_groups = 32,
        num_edges_eachgroup = 8
    ):
        super(FreeCGBlock, self).__init__()
        self.lmax = lmax
        ...
        assert num_groups * num_edges_eachgroup == hidden_channels, 'num_groups * num_edges_eachgroup should equal to hidden_channels'
        ...
        vis_mp_kwargs = dict(
            ...
            num_edges_eachgroup = num_edges_eachgroup
        )
...
class ViS_MP(MessagePassing):
    def __init__(
        ...
        self.num_edges_eachgroup = num_edges_eachgroup
        ...
        self.vec_edge_proj = nn.Linear(hidden_channels, num_heads)
        
        self.tp = o3.FullyConnectedTensorProduct( 
            irreps_in1=f'{num_edges_eachgroup}x1o + {num_edges_eachgroup}x2e',
            irreps_in2=f'{num_edges_eachgroup}x1o + {num_edges_eachgroup}x2e',
            irreps_out=f'{num_edges_eachgroup}x1o + {num_edges_eachgroup}x2e',
            shared_weights = False
        )
        self.tp_weight = nn.Parameter(torch.ones([1,num_groups,self.tp.weight_numel]))
    ...
    def reset_parameters(self):
        ...
        nn.init.xavier_uniform_(self.tp_weight)
        ...
        nn.init.xavier_uniform_(self.vec_edge_proj.weight)
        self.vec_edge_proj.bias.data.fill_(0)
    ...
    def forward(self, x, vec, edge_index, r_ij, f_ij, d_ij):
        ...
        dx = vec_dot * o2 + o3
        dvec_pre = vec3 * o1.unsqueeze(1) #+ vec_out N 8 256
        #dvec_pre = torch.permute(dvec_pre, (0, 2, 1)) # N 256 8
        
        num_head = self.num_groups
        head_dim = self.num_edges_eachgroup
        
        
        vec_out_ = torch.permute(vec_out, (0, 2, 1)) # N 256 8
        vec_out_ = vec_out_.reshape(vec_out_.size(0),num_head,head_dim,8) # N num_head head_dim 8
        vec_out_1 = vec_out_[:,:,:,:3].reshape(vec_out_.size(0),num_head,-1) # N num_head head_dim*3
        vec_out_2 = vec_out_[:,:,:,3:].reshape(vec_out_.size(0),num_head,-1) # N num_head head_dim*5
        vec_out_ = torch.concat([vec_out_1,vec_out_2],2) # N num_head head_dim*3+head_dim*5
        
        
        
        
        dot = torch.permute(dvec_pre,(0,2,1)) # N 8 256
        dot = dot.reshape(dot.size(0),num_head,head_dim,8) # N num_head head_dim 8
        dot_1 = dot[:,:,:,:3].reshape(dot.size(0),num_head,-1) # N num_head head_dim*3
        dot_2 = dot[:,:,:,3:].reshape(dot.size(0),num_head,-1) # N num_head head_dim*5
        dot = torch.concat([dot_1,dot_2],2) # N num_head head_dim*3+head_dim*5
        
        dot = self.tp(vec_out_, dot, self.tp_weight) # N num_head head_dim*3+head_dim*5
        
        dot_1 = dot[:,:,:head_dim*3].reshape(dot.size(0),num_head,head_dim,3) # N num_head head_dim 3
        dot_2 = dot[:,:,head_dim*3:].reshape(dot.size(0),num_head,head_dim,5) # N num_head head_dim 5
        
        dot = torch.concat([dot_1,dot_2],3).reshape(dot.size(0),num_head*head_dim,8) # N num_head*head_dim 8
        dot = torch.permute(dot,(0,2,1)) # N 8 num_head*head_dim
        dot = torch.concat([dot[:,:,num_head+int(num_head/2):],dot[:,:,:num_head+int(num_head/2)]],2)
        
        dvec = dot + vec_out# N 8 256
        if not self.last_layer:
    ...
    def message(self, q_i, k_j, v_j, vec_j, dk, dv, r_ij, d_ij):
        ...
        attn2 = (vec_j * d_ij.unsqueeze(-1)).sum(1,False) # N*N 256
        attn2 = self.vec_edge_proj(attn2) # N*N 8
        attn = (q_i * k_j * dk).sum(dim=-1) + attn2
